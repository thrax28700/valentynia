...
import { computeTotals, type FxLine } from '../../lib/facturx';
import { download } from '../../lib/download';
import { eur, eur2, dateShort } from '../../lib/format';
import { ppfStatusLabel } from '../../lib/labels';
...
export default function Billing() {
  ...
  const { data: journal = [] } = useVatJournal();
  const create = useCreateInvoice();
  const send = useSendToPpf();
  ...
  const outstanding = invoices.filter((i) => !i.paid).reduce((s, i) => s + i.totalInclVat, 0);
  const ppfOk = invoices.filter((i) => ['ACCEPTED', 'PAID'].includes(i.ppfStatus)).length;
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    await create.mutateAsync({
      ...
      clientSiren: head.clientSiren || undefined,
      issueDate: head.issued,
      dueDate: head.due,
      lines: lines.map((l) => ({
        label: l.label,
        quantity: l.qty,
  ...
  const dl = async (id: string, number: string) => {
    try {
      const xml = await fetchFacturxXml(id);
      download(`facturx-${number}.xml`, xml, 'application/xml');
    } catch {
      /* ignore */
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Stat
          ...
          delta={`${invoices.filter((i) => !i.paid).length} ouvertes`}
          tone="down"
        />
        <Stat
          label="Acceptées / encaissées PPF"
          value={String(ppfOk)}
          delta="Cycle de vie normalisé"
        />
        <Stat
          label="Chiffre d'affaires TTC"
          value={eur.format(invoices.reduce((s, i) => s + i.totalInclVat, 0))}
          delta="Cumul"
        />
      </div>
      ...
      <Card>
        <div className="mb-5 flex flex-wrap gap-1 rounded-full bg-wash p-1">
          {TABS.map(([k, label]) => (
            <button
              ...
              onClick={() => setTab(k)}
              className={cx(
                'rounded-full px-4 py-2 font-heading text-xs transition',
        ...
        {(tab === 'factures' || tab === 'ppf') && (
          <Table
            head={[
              ...
              'Montant TTC',
              'Émission',
              tab === 'ppf' ? 'Statut PPF' : 'Paiement',
              'Actions',
            ]}
          >
            {invoices.map((inv) => (
              <tr key={inv.id} className="text-prune hover:bg-wash">
                <td className="px-3 py-3 font-mono text-sm">{inv.number}</td>
                <td className="px-3 py-3">{inv.client}</td>
                <td className="px-3 py-3 font-mono text-sm">{eur2.format(inv.totalInclVat)}</td>
                <td className="px-3 py-3 font-mono text-xs text-mauve">{dateShort(inv.issueDate)}</td>
                <td className="px-3 py-3">
                  {tab === 'ppf' ? (
                    <StatusBadge status={ppfStatusLabel[inv.ppfStatus] ?? inv.ppfStatus} />
                  ) : (
                    <Badge tone={inv.paid ? 'sage' : 'neutral'}>
                ...
                <td className="px-3 py-3">
                  <div className="flex gap-1.5">
                    <button
                      ...
                      title="Télécharger le XML Factur-X"
                      onClick={() => dl(inv.id, inv.number)}
                    >
        ...
        {tab === 'archivage' && (
          <div className="grid gap-4 sm:grid-cols-2">
            {[
              {
                i: 'Lock' as const,
                t: 'Conservation 10 ans',
                d: 'Factures émises et reçues archivées à valeur probante, horodatées et scellées.',
              },
              {
                i: 'Shield' as const,
                t: 'Empreinte SHA-256',
                d: 'Chaque facture porte une empreinte vérifiable (visible dans le journal).',
              },
              {
                i: 'DocText' as const,
                t: 'Piste d’audit fiable',
                d: 'Lien commande → livraison → facture tracé de bout en bout.',
              },
              {
                i: 'Download' as const,
                t: 'Export sur demande',
                d: 'Export au format réglementaire en cas de contrôle.',
              },
            ].map((c) => (
        ...
        {tab === 'journal' && (
          <>
            <p className="mb-4 text-sm text-mauve">
              Journal chaîné : chaque écriture inclut l’empreinte de la précédente (inaltérabilité).
            </p>
            <Table head={['#', 'Événement', 'Empreinte écriture', 'Empreinte précédente', 'Date']}>
              {journal.map((e) => (
                <tr key={e.id} className="text-prune">
                  ...
                  <td className="px-3 py-3 font-mono text-xs text-mauve">{dateShort(e.createdAt)}</td>
                </tr>
              ))}
            </Table>
          </>
        ...
        {tab === 'relances' && (
          <div className="space-y-3">
            {[
              {
                step: 'J+3 après échéance',
                text: 'Rappel courtois par e-mail, facture Factur-X jointe.',
              },
              {
                step: 'J+10',
                text: 'Relance ferme + proposition d’échéancier. Notification au commercial.',
              },
              {
                step: 'J+21',
                text: 'Mise en demeure générée automatiquement, à valider avant envoi.',
              },
            ].map((r) => (
        ...
        <form className="space-y-4" onSubmit={submit}>
          <div className="grid grid-cols-2 gap-4">
            <Field
              label="Client"
              value={head.client}
              onChange={(v) => setHead({ ...head, client: v })}
              required
            />
            <Field
              label="SIREN client (option)"
              value={head.clientSiren}
              onChange={(v) => setHead({ ...head, clientSiren: v })}
            />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <Field
              ...
              type="date"
              value={head.issued}
              onChange={(v) => setHead({ ...head, issued: v })}
              required
            />
            <Field
              label="Échéance"
          ...
          <div className="space-y-2">
            ...
            {lines.map((l, i) => (
              <div key={i} className="grid grid-cols-[1fr_64px_92px_64px_32px] items-center gap-2">
                ...
                />
                <input
                  className="v-input text-right font-mono"
                  type="number"
                  min="0"
                  step="0.01"
                  value={l.qty}
          ...
          <div className="rounded-xl bg-wash p-3 text-sm">
            <div className="flex justify-between text-mauve">
              <span>Total HT</span>
              <span className="font-mono">{eur2.format(totals.totalHT)}</span>
            </div>
            <div className="flex justify-between text-mauve">
              <span>TVA</span>
              <span className="font-mono">{eur2.format(totals.tva)}</span>
            </div>
            <div className="mt-1 flex justify-between font-heading font-semibold text-prune">
              <span>Total TTC</span>
              <span className="font-mono">{eur2.format(totals.totalTTC)}</span>
            </div>
          </div>
          ...
          <div className="flex justify-end gap-2 pt-1">
            ...
            <Button type="submit" icon="Receipt" loading={create.isPending}>
              Générer la facture
            </Button>
          </div>
        </form>
